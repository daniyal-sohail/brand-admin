...
const EditTemplateModal = ({ template, isOpen, onClose, onSuccess }) => {
  const { putTemplateData, templateLoading } = useAdminTemplate();
  ...
  useEffect(() => {
    if (template) {
      setFormData({
        ...
        canvaTemplateUrl: template.canvaTemplateUrl || ""
      });
      setErrors({});
    }
  }, [template]);
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await putTemplateData(`/admin/templates/${template._id}`, payload);
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            <div className="bg-brand-beige p-2 rounded-lg">
              <Edit3 className="w-5 h-5 text-brand-warm-brown" />
            </div>
            <div>
              <h2 className="text-xl font-semibold text-brand-charcoal">Edit Template</h2>
              <p className="text-sm text-brand-warm-brown">Update template information</p>
            </div>
          </div>
          <button
            onClick={onClose}
